...
const FEEDBACK: Record<GoNoGoOutcome, { text: (rt: number | null) => string; tone: string }> = {
  HIT: { text: (rt) => `Hit! ${rt === null ? '' : formatMs(rt)}`.trim(), tone: 'good' },
  CORRECT_REJECTION: { text: () => 'Held back. Good.', tone: 'good' },
  COMMISSION: { text: () => 'Tapped on STOP!', tone: 'bad' },
  OMISSION: { text: () => 'Missed the GO', tone: 'bad' },
  FALSE_START: { text: () => 'Too early!', tone: 'warn' },
};
...
export const goNoGoScreen: ScreenFactory = (ctx, state) => {
  ...
  const tap = h('div', { class: 'tap-area', part: 'tap-area', role: 'button', tabindex: '0', 'aria-label': 'Go / Stop signal', 'data-phase': 'waiting' }, label, hint);
  ...
  function patch(next: MachineState): void {
    ...
    const commissions = progress.trials.filter((t) => t.outcome === 'COMMISSION' || t.outcome === 'FALSE_START').length;
    const omissions = progress.trials.filter((t) => t.outcome === 'OMISSION').length;
    setText(counter, `Signal ${Math.min(shown, total)} / ${total}`);
    setText(counts, `False taps ${commissions}/${config.gngMaxCommissions} · Missed ${omissions}/${config.gngMaxOmissions}`);
    ...
    switch (stage.type) {
      case 'GNG_AWAITING':
        ...
        setText(label, 'Get ready…');
        setText(hint, 'Tap on green GO. Hold back on red STOP.');
        setText(feedback, '');
      ...
      case 'GNG_STIMULUS_ACTIVE':
        ...
        delete tap.dataset['outcome'];
        setText(label, stage.isGo ? 'GO' : 'STOP');
        setText(hint, '');
        break;
      case 'GNG_RESULT_DISPLAY': {
        const fb = FEEDBACK[stage.outcome];
        ...
        setText(label, fb.text(stage.rtMs));
        setText(hint, '');
        setText(feedback, fb.text(stage.rtMs));
